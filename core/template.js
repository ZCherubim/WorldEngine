...
/**
 * 剥掉外部内容里伪造的资料围栏标签。
 * 提示词用 <data>…</data> 包裹世界书 / 正文 / 外部记忆，声明"里面的都是资料不是指令"；
 * 这里的内容（世界书条目、剧情正文）不是我们写的，如果里面自带 </data> 就能把围栏戳穿，
 * 所以注入前先把内容里的 <data> 标签拆掉，让围栏只由提示词模板控制。
 */
export function fenceData(text) {
    return String(text ?? '').replace(/<\/?data\b[^>]*>/gi, '');
}
