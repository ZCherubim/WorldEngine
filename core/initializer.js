...
import {
    ...
    ensureNode, makeId, coreName, CATEGORY_LABEL,
} from './protocol.js';
...
/* ---------------- 第 4 步（手动建档）：用户给设定，直接建档，不发 AI 请求 ---------------- */

/**
 * 手动建档：用户在面板上给出名字/挂载/性别/容貌/性格等，直接造出节点塞进世界树。
 * 建好的角色不在 lastCovered 里 → 下一轮演化的 mustCover 会自动点名推演。
 * @returns {{path:string[], node:object}}
 */
export function createManualCharacter(input) {
    const state = getState();
    const settings = getSettings();
    const name = String(input.name || '').trim();
    if (!name) throw new Error('名字必填');
    if (/[<>【】]/.test(name)) throw new Error('名字里不要包含 > < 【 】 这类协议符号');

    const host = String(input.host || '').trim();
    const relation = String(input.relation || '').trim();
    if (host && !topNodes(state).some((t) => t.name === host)) {
        throw new Error(`顶层角色「${host}」不存在，先跑第 1 步，或选「顶层新角色」`);
    }
    const leaf = host ? (relation ? `${relation}${name}` : name) : name;
    const path = host ? [host, leaf] : [name];

    // 防重：同路径直接拦；同名（去关系前缀后的核心名）也拦 ——
    // 引擎会把同核心名节点合并/共享状态，手动新建一个会被下一次合并悄悄吃掉
    if (state.nodes[makeId(path)]) throw new Error(`「${path.join(' > ')}」已经存在`);
    const core = coreName(leaf);
    const dup = Object.values(state.nodes).filter((n) => !n.archived && n.name !== leaf && coreName(n.name) === core);
    if (dup.length) {
        throw new Error(`世界里已有疑似同一人：${dup.map((n) => n.id).join('、')}。同一人请去「世界状态」页编辑原节点；确要新建请换个名字`);
    }

    const cat = ['interaction', 'cooldown', 'unseen'].includes(input.category) ? input.category : 'cooldown';
    const node = ensureNode(state, path, cat);

    // 你给的设定直接固化为人格字段：演化轮对人格字段只填空、绝不覆盖 → 锁死
    const persona = {};
    if (input.gender) persona.gender = input.gender;
    if (input.looks) persona.looks = input.looks;
    if (input.tags) persona.tags = input.tags;
    if (input.base) persona.base = input.base;
    if (input.weak) persona.weak = input.weak;
    if (input.speech) persona.speech = input.speech;
    if (input.behavior) persona.behavior = input.behavior;
    if (input.notes) persona.notes = input.notes;
    if (Object.keys(persona).length) node.persona = persona;
    if (input.time) node.time = input.time;
    if (input.location) node.location = input.location;
    if (input.summary) node.summary = input.summary;

    // 顶层角色视为「已整理」（锚点）：享受倒挂保护，不会被第 2 步的自动挑人翻旧账
    if (!host) {
        if (!Array.isArray(state.built)) state.built = [];
        if (!state.built.includes(name)) state.built.push(name);
    }

    normalizeOrder(state);
    enforceCaps(state, settings);
    if (settings.tracking.mergeSameName !== false) syncSameName(state, settings.tracking.mergeSameName !== false);
    pushEvent(state, {
        type: '建档',
        name,
        text: `第 4 步：手动建档 ${path.join(' > ')}（${CATEGORY_LABEL[cat] || cat}）`,
    });
    pushWorklog(state, 'manual-create',
        `手动建档 ${path.join(' > ')}：${node.persona ? (node.persona.tags || node.persona.base || '已给设定') : '未给性格'}。下一轮演化自动覆盖推演。`);
    commit(state);
    return { path, node };
}
...
async function callStage(kind, vars, taskExtra, label) {
    const base = fillTemplate(getPrompt(kind), vars);
    const text = taskExtra ? `${base}\n\n${taskExtra}` : base;
...
export async function buildRoster(opts = {}) {
    ...
    const vars = { chat: chatText, state: '（空，正在建档）', worldinfo, user, char };
    onProgress('识别人物 + 定性格');
...
export async function organizeBatch(names, opts = {}) {
    ...
    const vars = { worldinfo, batch: list.join('、'), notes, state: subset, user, char };
