...
import { escapeHtml, toast, getNames } from '../core/st.js';
...
import {
    ...
    stateLine, PERSONA_ORDER, PERSONA_LABEL, BOND_TIERS,
} from '../core/protocol.js';
import { extractHour, timeOfDay } from '../core/timeline.js';
import { initializeWorld, buildRoster, organizeBatch, finalizeOrganize, probeWorldBook, createManualCharacter } from '../core/initializer.js';
...
import { getLastInject, injectStatusText } from '../core/injector.js';
...
let buildStatus = '';
let mcStatus = '';   // 第 4 步手动建档的成功提示（渲染时显示；下次点建档时清掉）
...
function renderBuild() {
    ...
    /* ---- 第 4 步：手动建档（永远显示）---- */
    out.push(`<div class="we-card">
        <div class="we-row"><span class="we-name">第 4 步｜手动建档（自己造一个角色塞进世界）</span></div>
        <div class="we-sub">你给设定，引擎直接建档——<b>不走 AI、不发请求、不需要世界书条目</b>。建好的人立刻出现在世界状态里，下一轮演化自动开始推演（想马上推就点底部「立即更新」）。给过的设定（性格/容貌/备注等）在演化中<b>锁死</b>，AI 不会篡改。</div>
        <div class="we-grid2">
            <div class="we-field"><label>名字（必填）</label><input type="text" id="mcf-name" placeholder="如：林晚"></div>
            <div class="we-field"><label>挂在谁下面</label><select id="mcf-host">
                <option value="">顶层新角色（和女主平级）</option>
                ${tops.map((t) => `<option value="${escapeHtml(t.name)}">${escapeHtml(t.name)} 的关系人</option>`).join('')}
            </select></div>
        </div>
        <div class="we-grid2">
            <div class="we-field"><label>关系称谓（挂载时填）</label><input type="text" id="mcf-relation" placeholder="如：母亲 / 闺蜜 / 追求者，留空 = 直接用名字"></div>
            <div class="we-field"><label>初始类别</label><select id="mcf-cat">
                <option value="cooldown" selected>冷却（在世界里生活，剧情暂未聚焦）</option>
                <option value="interaction">交互（正在戏里）</option>
                <option value="unseen">未出场</option>
            </select></div>
        </div>
        <div class="we-field"><label>容貌（可空）</label><input type="text" id="mcf-looks" placeholder="如：及肩黑发，笑起来有梨涡，常穿浅色连衣裙"></div>
        <div class="we-grid2">
            <div class="we-field"><label>性别（可空）</label><select id="mcf-gender"><option value="">（不填）</option><option>女</option><option>男</option><option>其他</option></select></div>
            <div class="we-field"><label>性格标签（可空）</label><input type="text" id="mcf-tags" placeholder="如：温柔 · 嘴硬心软 · 报复心强"></div>
        </div>
        <div class="we-field"><label>底色（可空：年龄 / 出身 / 职业 / 处境一句话）</label><input type="text" id="mcf-base" placeholder="如：29岁，县城开花店的，和丈夫分居中"></div>
        <div class="we-grid2">
            <div class="we-field"><label>软肋（可空）</label><input type="text" id="mcf-weak" placeholder="如：最怕别人提她的婚事"></div>
            <div class="we-field"><label>说话方式（可空）</label><input type="text" id="mcf-speech" placeholder="如：慢条斯理，爱用反问"></div>
        </div>
        <div class="we-grid2">
            <div class="we-field"><label>行为习惯（可空）</label><input type="text" id="mcf-behavior" placeholder="如：习惯性摩挲无名指上的戒指"></div>
            <div class="we-field"><label>其他备注（可空）</label><input type="text" id="mcf-notes" placeholder="任何想固定的设定：身世、秘密、禁忌…"></div>
        </div>
        <div class="we-grid2">
            <div class="we-field"><label>初始时间段（可空）</label><input type="text" id="mcf-time" placeholder="如 09:00-18:00，留空 = 下一轮演化时定"></div>
            <div class="we-field"><label>初始地点（可空）</label><input type="text" id="mcf-loc" placeholder="如：城南花语花店"></div>
        </div>
        <div class="we-field"><label>初始状态（可空：此刻在做什么）</label><input type="text" id="mcf-summary" placeholder="如：在店里修剪玫瑰，等下午的送货"></div>
        <div class="we-row" style="margin-top:6px">
            <button class="we-primary" data-act="mc-create">④ 建档加入世界</button>
            ${mcStatus ? `<span class="we-sub" style="color:#3c9">✓ ${escapeHtml(mcStatus)}</span>` : ''}
        </div>
    </div>`);
...
/** 心情 / 目标 / 近况小行（内心面；缺的就不显示） */
function moodText(n) {
    return [n.mood && `心情：${n.mood}`, n.goal && `目标：${n.goal}`, n.mindset && `近况：${n.mindset}`].filter(Boolean).join('　');
}

/**
 * 心线 chips（v2.4.0）：她对谁是什么档，点击弹出调整。
 * {{user}} 存储键显示时用真名美化，data-target 保留原键。
 */
function bondChips(n) {
    const bonds = Object.entries(n.bonds || {});
    if (!bonds.length) return '';
    const { user } = getNames();
    return bonds.map(([k, b]) => {
        if (!b || !Number.isFinite(b.tier)) return '';
        const tgt = k === '{{user}}' ? (user || '你') : k;
        return `<span class="we-bond" data-act="bond-edit" data-name="${escapeHtml(n.name)}" data-target="${escapeHtml(k)}" title="点击调整心线档位">${escapeHtml(tgt)}·${escapeHtml(BOND_TIERS[b.tier] || '？')}</span>`;
    }).filter(Boolean).join('');
}
...
function stateBlock(n, hidden, tagsHtml) {
    ...
    return `<div class="we-line we-stack">
        ...
        ${bonds ? `<div class="we-row" style="flex-wrap:wrap;gap:4px">${bonds}</div>` : ''}
    </div>`;
}
...
function renderState() {
    ...
    // 生效承诺（v2.4.0）：正文里说出口的保证 —— 攻防裁判的另一半，违背自动触发心线惩罚
    const commits = (state.commitments || []).filter((c) => c && c.status === 'active');
    if (commits.length) {
        out.push(`<details class="we-fold" style="margin-top:4px">
            <summary>生效承诺（${commits.length}）<span class="we-sub">违背会触发心线惩罚，点 ✕ 解除（解除不算违背）</span></summary>
            ${commits.slice(0, 20).map((c) => `<div class="we-line we-stack">
                <div class="we-row"><span class="we-name">${escapeHtml(c.who)}</span>
                    <span class="we-sub">第 ${c.since || 0} 层立下${c.at ? ` ｜ ${escapeHtml(c.at)}` : ''}</span>
                    <span class="we-eye" data-act="commit-release" data-cid="${escapeHtml(c.id || '')}" title="解除这条承诺（不算违背，不罚心线）" style="cursor:pointer;margin-left:auto">✕</span></div>
                <div class="we-text">「${escapeHtml(c.text)}」</div>
            </div>`).join('')}
        </details>`);
    }
...
function renderSettings() {
    ...
    return `<div class="we-form">
        ...
        <div class="we-field"><label>请求超时（秒；gemini 系慢中转建议 300 以上，实测单轮可达 100+ 秒）</label><input type="number" min="30" step="30" id="we-api-timeout" value="${Math.round((s.api.timeoutMs || 300000) / 1000)}"></div>
        ...
        <div class="we-field" style="margin-left:22px"><label>攻防停滞阈值（关系线对手几层没出手就被硬点名）</label><input type="number" min="1" max="20" id="we-rival-gap" value="${Number(s.tracking.rivalGap) || 3}"></div>
        ...
        <div class="we-field"><label>注入强度（v2.4.0）</label><select id="we-inject-mode">
            <option value="passive" ${(s.inject.mode || 'active') === 'passive' ? 'selected' : ''}>被动背景 —— 只给动向/现场内心/性格，最省 token</option>
            <option value="active" ${(s.inject.mode || 'active') === 'active' ? 'selected' : ''}>主动钩子（默认）—— 追加心线承诺、点名衔接、剧情走向（"可以走进剧情"）</option>
            <option value="director" ${(s.inject.mode || 'active') === 'director' ? 'selected' : ''}>强导演 —— 钩子升级为硬指令（"接下来的正文应让这些事件发生"）</option>
        </select></div>
        <div class="we-hint" id="we-inject-status">${injectStatusText(getLastInject())}</div>
...
function collectSettings() {
    // v2.3.27：设置页不在 DOM 时（当前渲染的是别的页签），什么都不动 ——
    // 旧代码会把不存在的输入读成空值：在别的页签勾个复选框就把 API 地址/密钥整个洗掉了
    if (!document.getElementById('we-api-url')) return;
    const s = getSettings();
    const val = (id) => { const el = document.getElementById(id); return el ? el.value : ''; };
    const chk = (id) => { const el = document.getElementById(id); return el ? el.checked : false; };
    const num = (id, d) => { const v = Number(val(id)); return Number.isFinite(v) ? v : d; };
    s.enabled = chk('we-enabled');
    s.autoUpdate = chk('we-auto');
    s.allowFallback = chk('we-fallback');
    s.debug = chk('we-debug');
    // 外观（读不到控件时保持原值，别把用户选择清掉）
    s.ui = s.ui || {};
    if (document.getElementById('we-theme')) s.ui.theme = val('we-theme') || 'auto';
    s.api.mode = val('we-api-mode') || 'tavern';
    s.api.tavernProfile = val('we-tavern-profile') || '';
    s.api.url = val('we-api-url').trim();
    s.api.key = val('we-api-key').trim();
    s.api.model = val('we-api-model').trim();
    s.api.temperature = num('we-api-temp', 0.45);
    s.api.maxTokens = Math.max(256, num('we-api-maxtokens', 4096));
    if (document.getElementById('we-api-timeout')) s.api.timeoutMs = Math.max(30, num('we-api-timeout', 300)) * 1000;
    if (document.getElementById('we-include-preset')) s.api.includePreset = chk('we-include-preset');
    s.tracking.updateDepth = num('we-update-depth', 3);
    s.tracking.initDepth = num('we-init-depth', 40);
    s.tracking.injectionDepth = num('we-inject-depth', 2);
    s.tracking.cooldownThreshold = num('we-cooldown', 20);
    s.tracking.interactionLimit = num('we-int-limit', 0);
    s.worldbook.enabled = chk('we-wb-enabled');
    s.worldbook.name = val('we-wb-name').trim();
    s.worldbook.includeNsfw = chk('we-wb-nsfw');
    s.worldbook.organizeDropOverview = chk('we-wb-dropover');
    s.worldbook.maxChars = num('we-wb-maxchars', 0);
    s.tracking.batchSize = Math.max(1, num('we-batch-size', 3));
    s.tracking.noteCount = Math.max(0, num('we-note-count', 3));
    s.tracking.autoFill = chk('we-autofill');
    s.tracking.fillMinMissing = Math.max(1, Math.min(30, num('we-fillmin', 3)));
    s.tracking.autoFallback = chk('we-autofallback');
    s.tracking.mergeSameName = chk('we-mergesame');
    s.tracking.mergeDuplicates = chk('we-mergedup');
    s.tracking.timeSlot = chk('we-timeslot');
    s.tracking.sceneGuard = chk('we-sceneg');
    s.tracking.sceneDepth = Math.max(1, Math.min(6, num('we-scened', 2)));
    s.tracking.rivalGap = Math.max(1, Math.min(20, num('we-rival-gap', 3)));
    s.tracking.autoRollback = chk('we-auto-rollback');
    s.tracking.snapshots = chk('we-snapshots');
    s.memory.eventLimit = Math.max(0, num('we-mem-events', 0));
    s.memory.momentCount = Math.max(0, num('we-mem-moments', 3));
    s.memory.maxChars = Math.max(0, num('we-mem-maxchars', 0));
    s.memory.batchEvents = Math.max(0, num('we-mem-batch', 0));
    ...
    s.writeToChat.enabled = chk('we-write-enabled');
    s.writeToChat.auto = chk('we-write-auto');
    s.writeToChat.topN = Math.min(10, Math.max(1, num('we-write-topn', 3)));
    s.writeToChat.categories = {
        interaction: chk('we-wc-interaction'),
        cooldown: chk('we-wc-cooldown'),
        unseen: chk('we-wc-unseen'),
    };
    s.inject.enabled = chk('we-inject-enabled');
    s.inject.persona = chk('we-inject-persona');
    s.inject.skipWritten = chk('we-inject-skip');
    s.inject.position = val('we-inject-pos');
    s.inject.mode = val('we-inject-mode') || 'active';
    saveSettings();
    return feedMissing;
}
...
async function handleAction(act, el) {
    ...
    if (act === 'bond-edit') {
        ...
        const st = getState();
        const n = Object.values(st.nodes || {}).find((x) => x.name === name);
        if (!n || !n.bonds || !n.bonds[target]) return;
        const cur = BOND_TIERS[n.bonds[target].tier] || '陌生';
        const input = window.prompt(
            `调整 ${name} 对 ${target === '{{user}}' ? '你' : target} 的心线（当前：${cur}）\n`
            + `输入档位（${BOND_TIERS.join(' / ')}），留空取消`,
            cur,
        );
        if (input == null || !String(input).trim()) return;
        const tier = BOND_TIERS.indexOf(String(input).trim());
        if (tier === -1) { toast(`没认出这个档位，七档是：${BOND_TIERS.join(' / ')}`, 'error'); return; }
        n.bonds[target] = { ...n.bonds[target], tier, floor: getChatLength(), at: new Date().toLocaleString('zh-CN') };
        commit(st);
        render();
        return;
    }
    if (act === 'commit-release') {
        const cid = el.dataset.cid;
        const st = getState();
        const c = (st.commitments || []).find((x) => x.id === cid);
        if (!c) return;
        if (!confirm(`解除 ${c.who} 的承诺「${c.text}」？\n（解除 = 双方谈妥后不再算数，不触发心线惩罚；违背才会）`)) return;
        c.status = 'released';
        c.at = new Date().toLocaleString('zh-CN');
        commit(st);
        render();
        return;
    }
    ...
    if (act === 'mc-create') {
        ...
        mcStatus = '';
        try {
            const res = createManualCharacter({
                name: g('mcf-name'),
                host: g('mcf-host'),
                relation: g('mcf-relation'),
                category: g('mcf-cat'),
                gender: g('mcf-gender'),
                looks: g('mcf-looks'),
                tags: g('mcf-tags'),
                ...
                weak: g('mcf-weak'),
                speech: g('mcf-speech'),
                behavior: g('mcf-behavior'),
                notes: g('mcf-notes'),
                time: g('mcf-time'),
                location: g('mcf-loc'),
                summary: g('mcf-summary'),
            });
            mcStatus = `已建档「${res.path.join(' > ')}」${res.path.length > 1 ? '，下一轮演化会自动推演' : '，可点底部「立即更新」马上推演'}`;
            render(true);
        } catch (e) {
            toast(`建档失败：${e.message}`, 'error');
        }
        return;
    }
    if (act === 'memory-clear') {
