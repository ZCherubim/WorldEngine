 ...
 * 注入器需要跳过它们，避免世界状态被塞进演化提示词自己（token 翻倍 + 自我污染）。
 *
 * v2.3.28 竞态修复：旧版注入器只看全局忙标志（isSelfGenerating），它分不清
 * 「这次请求就是演化自己」和「演化恰好在跑、来的是用户的正文请求」——后者会被
 * 一票否决、静默跳过注入，赶在演化完成前发的消息全部裸奔（世界状态根本进不了上下文）。
 * 现在改按内容识别：callLLM 给插件自己的每个请求尾部追加一段零宽字符签名（SELF_TAG），
 * 注入器查签名命中才跳过；演化运行期间用户的正文注入不再被殃及。
 */
let selfGenerating = 0;

export function isSelfGenerating() { return selfGenerating > 0; }

/** 自调用签名：零宽字符序列，肉眼不可见，酒馆的宏替换不会碰它 */
export const SELF_TAG = '\u200b\u2060\u200b\u2060\u200b\u2060';

/** 给消息数组最后一条的尾部追加自调用签名（浅拷贝，不动调用方传进来的对象） */
function tagSelfMessages(messages) {
    const arr = Array.isArray(messages) ? messages : [];
    if (!arr.length) return arr;
    const last = arr[arr.length - 1];
    if (!last || typeof last !== 'object') return arr;
    const content = String(last.content || '');
    if (content.includes(SELF_TAG)) return arr;
    return arr.slice(0, -1).concat([{ ...last, content: content + SELF_TAG }]);
}
...
async function tavernChat(messages, timeoutMs = 180000) {
    ...
    const temperature = Number(s.api.temperature) || 0.4;
    const errs = [];
...
async function openaiChatOnce(endpoint, key, model, messages, temperature, maxTokens, timeoutMs) {
    ...
    try {
        ...
        const content = textOf(pickContent(data));
        if (content) {
            // v2.3.26：思考型模型（gemini-3 系等）的思考 token 会吃掉 max_tokens 预算，
            // 可见输出可能远小于请求值就被掐断（实测 4096 只出 ~1300）。
            // 这里把截断标记带回上层，由 openaiChat 决定是否加大预算重试。
            const truncated = data?.choices?.[0]?.finish_reason === 'length';
            return { ok: true, text: content, status: res.status, truncated };
        }
        return { ok: false, err: extractError(data, res.status), status: res.status };
    } catch (e) {
        // AbortController 掐断时浏览器只给 "signal is aborted without reason"，
        // 用户完全看不出是超时 —— 翻译成人话
        const aborted = !!(e && (e.name === 'AbortError' || /abort/i.test(e.message || '')));
        const err = aborted
            ? `请求超时（${Math.round(timeoutMs / 1000)} 秒）。该接口响应较慢，可在「接口」页把超时调大，或换更快的模型`
            : ((e && e.message) || String(e));
        return { ok: false, err, status: 0, isTimeout: aborted };
    } finally {
        clearTimeout(timer);
    }
}

async function openaiChat(url, key, model, messages, temperature, maxTokens, timeoutMs = 300000) {
    const base = ensureBasePath(stripKnownEndpoint(url));
    const endpoint = base ? `${base}/chat/completions` : '';
    dbg('独立 API 请求 →', endpoint, '| model =', model);

    let first = await openaiChatOnce(endpoint, key, model, messages, temperature, maxTokens, timeoutMs);

    // 网络层错误（Failed to fetch / 连接被重置等）：中转站偶发抽风，等 2 秒重试一次
    if (!first.ok && first.status === 0 && !first.isTimeout) {
        dbg('独立 API 网络错误（', first.err, '）→ 2 秒后重试一次');
        await new Promise((r) => setTimeout(r, 2000));
        const retry = await openaiChatOnce(endpoint, key, model, messages, temperature, maxTokens, timeoutMs);
        if (retry.ok) first = retry;
        else throw new Error(retry.err || `HTTP ${retry.status}`);
    }
    ...
    // 输出被截断（finish_reason=length）：思考 token 吃掉了预算 → 加大 max_tokens 重试一次。
    // 重试仍截断就返回现有内容（解析层会丢弃写了一半的行，其余照常生效），但要打警告让用户知道。
    if (first.ok && first.truncated) {
        const bigger = Math.min((Number(maxTokens) || 4096) * 4, 65536);
        dbg(`独立 API 输出被截断（finish_reason=length）→ 用 max_tokens=${bigger} 重试一次`);
        const second = await openaiChatOnce(endpoint, key, model, messages, temperature, bigger, timeoutMs);
        if (second.ok) {
            if (second.truncated) {
                console.warn('[WorldEngine] 输出仍被截断（max_tokens=' + bigger + '）。建议在「接口」页调大「最大输出 Token」');
            }
            return second.text;
        }
        console.warn('[WorldEngine] 截断重试失败，使用首次的部分结果（末尾可能缺人）：', second.err);
        return first.text;
    }
...
export async function callLLM(messages, opts = {}) {
    const s = getSettings();
    const mode = (s.api && s.api.mode) || 'tavern';
    const tagged = tagSelfMessages(messages); // 尾部打上自调用签名，注入器按它识别"这是我们自己发的请求"
    selfGenerating++;
    try {
        return await callLLMInner(tagged, opts, s, mode);
    } finally {
        selfGenerating--;
    }
}
...
async function callLLMInner(messages, opts, s, mode) {
    ...
    if (apiReady()) {
        try {
            // 超时优先级：调用方显式指定（如自检的 90 秒）> 设置页的「请求超时」> 300 秒兜底
            const timeoutMs = opts.timeoutMs || Number(s.api.timeoutMs) || 300000;
            return await openaiChat(s.api.url, s.api.key, s.api.model, messages, s.api.temperature, s.api.maxTokens, timeoutMs);
        } catch (e) {
            lastErr = e;
